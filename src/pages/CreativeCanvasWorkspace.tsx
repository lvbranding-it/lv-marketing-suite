...
export default function CreativeCanvasWorkspace() {
  ...
  const requiredProviderCapability = operation === "edit_image" ? "image_edit"
    : operation === "generate_image" || operation === "variations" ? "image_generation"
      : operation === "visual_critique" ? "vision" : "text";
  const eligibleProviders = enabledProviders.filter((item) => item.capabilities.includes(requiredProviderCapability));
  const chosenProviderAvailable = provider === "auto"
    ? eligibleProviders.length > 0
    : eligibleProviders.some((item) => item.id === provider);
  const providerBlocker = providerStatusData && !chosenProviderAvailable ? {
    title: isImageOperation ? "Image generation is unavailable" : "That AI provider is unavailable",
    description: provider === "auto"
      ? `No enabled provider currently supports ${requiredProviderCapability.replaceAll("_", " ")}. An administrator needs to check the AI provider secrets and model configuration.`
      : `${PROVIDER_LABELS[provider] ?? provider} does not currently support this action. Choose Auto or another enabled provider.`,
  } : null;
  ...
  const generationBlocker = providerBlocker ?? imageBlocker;
