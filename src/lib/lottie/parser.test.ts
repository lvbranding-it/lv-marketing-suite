...
describe("Lottie recoloring", () => {
  it("recolors all normalized matches in a deep clone and preserves alpha and unrelated data", () => {
    ...
    expect(report.appliedOccurrenceCount).toBe(2);
    expect(report.appliedByOriginalColor).toEqual({ "#6D6A88": 2 });
    ...
    expect(items[2].c.k).toEqual([{ t: 0, s: [109 / 255, 106 / 255, 136 / 255, 1] }]);
