...
import { AlertTriangle, Check, Globe2, Loader2, Radar, RotateCcw } from "lucide-react";
    ...
    return (
      <div className="mx-auto flex min-h-[68vh] w-full max-w-3xl items-center px-4 py-12 sm:px-6">
        <section ref={errorRef} role="alert" tabIndex={-1} className="w-full rounded-2xl border border-black/10 bg-white p-6 text-center outline-none shadow-[0_15px_45px_rgba(35,31,32,.08)] sm:p-10">
          ...
          <h1 className="mt-5 text-2xl font-semibold tracking-[-0.025em]">{copy.analyzing.failedHeading}</h1>
    ...
    <div className="mx-auto flex min-h-[72vh] w-full max-w-5xl items-center px-4 py-12 sm:px-6">
      <div className="grid w-full gap-10 lg:grid-cols-[.9fr_1.1fr] lg:items-center">
        <div className="relative mx-auto aspect-square w-full max-w-[390px]" aria-hidden="true">
          ...
          <div className="absolute inset-[20%] rounded-full border border-dashed border-black/15" />
          <div className="absolute inset-[33%] rounded-full border border-black/10" />
          <div className="audit-radar-sweep absolute inset-[7%] rounded-full" />
          <div className="absolute left-1/2 top-1/2 flex h-24 w-24 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full bg-lv-charcoal text-white shadow-xl">
            <Radar size={39} strokeWidth={1.5} />
          </div>
          {["left-[17%] top-[24%]", "right-[14%] top-[40%]", "bottom-[18%] left-[30%]"].map((position, index) => (
            <span key={position} className={`audit-pulse-dot absolute ${position} h-3 w-3 rounded-full bg-primary`} style={{ animationDelay: `${index * 650}ms` }} />
          ))}
        </div>
        ...
        <section>
          ...
          <h1 className="mt-4 text-3xl font-semibold leading-tight tracking-[-0.035em] sm:text-4xl">{copy.analyzing.heading}</h1>
