...
Representative pages are discovered from the submitted page's navigation. When that yields nothing, which is the normal outcome on site builders that render navigation in JavaScript, the crawler falls back to `sitemap.xml` and then `sitemap_index.xml`. A sitemap index is split by content type and the general pages are rarely its first entry, so child sitemaps whose name identifies them as pages are read first and at most two are opened. The fallback runs only when in-page discovery found nothing, so sites that already expose their navigation pay no extra request. An audit that still finds no representative pages records a `no_representative_pages` warning and the report discloses the reduced scope.
