...
function endpoint(name: string, validSession = true, member = false) {
  let handler!: (request: Request) => Promise<Response>;
  const calls: string[] = [];
  const db = {
    auth: { getUser: async () => ({ data: { user: validSession ? { id: "alice" } : null }, error: null }) },
    from: (table: string) => {
      calls.push(table);
      const query = {
        select: () => query,
        eq: () => query,
        maybeSingle: async () => ({ data: table === "team_members" && member ? { user_id: "alice" } : null, error: null }),
      };
      return query;
    },
  };
  ...
  const provider = vi.fn(() => { throw new Error("Provider must not be called"); });
  const requireModule = (id: string) => {
    if (id.includes("http/server")) return { serve: (fn: typeof handler) => { handler = fn; } };
    if (id.includes("supabase-js")) return { createClient: () => db };
    if (id.includes("ai-authorization")) return authorization;
    throw new Error(`Unexpected import ${id}`);
  };
  new Function("require", "exports", "Deno", "fetch", compiled)(requireModule, {}, { env: { get: () => "configured" } }, provider);
  return { handler, calls, provider };
}

const orgId = "11111111-1111-4111-8111-111111111111";
const projectId = "22222222-2222-4222-8222-222222222222";
const body = { orgId, projectId, agentId: "lead_intel_v1", input: "Help", skillSystemPrompt: "Help", userMessage: "Help" };
function request(payload: object = body) {
  return new Request("https://example.test", { method: "POST", headers: { Authorization: "Bearer forged-or-valid", "Content-Type": "application/json" }, body: JSON.stringify(payload) });
}
...
describe.each(["agent-run", "skill-run"])("%s request boundary", name => {
  ...
  it("rejects missing organization scope", async () => {
    ...
    expect((await app.handler(request({ ...body, orgId: undefined }))).status).toBe(400);
    expect(app.calls).toEqual([]);
    expect(app.provider).not.toHaveBeenCalled();
  });
});
